 ...
 */
namespace FatFree\Helpers\AlternativeGetTextTranslator;
...
{
    private $domainSelectorInKeys = false;
    private $_vars = array();
    ...
    public function __construct($cachePath = false, $domainSelectorInKeys = false)
    {
        $this->setCachePath($cachePath);
        $this->useDomain("messages"); // PHP bug #21965
        $this->domainSelectorInKeys = $domainSelectorInKeys;
    }
    ...
    public function translate($key, $htmlencode = true)
    {
        ...
        if ($this->domainSelectorInKeys && count($explodedKey) > 1) {
            $this->useDomain($explodedKey[0]);
            $key = $explodedKey[1];
        }
